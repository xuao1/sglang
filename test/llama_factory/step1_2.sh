...
for batch_val in 1 4 16 32 64; do
    ...
    for stream_a_val in 12 24 32 44 52 64 76 88 96; do       
        ...
        sed -i "s/\(stream_a, stream_b = freeslots.create_greenctx_stream_by_value(\)[0-9.]\+/\1$stream_a_val/" /workspace/sglang/python/sglang/bench_one_batch.py
        
        # 输出当前参数到日志文件
        echo "===== Batch: $batch_val, Stream_a: $stream_a_val ===== [时间: $(date +'%T')]"
        echo "===== Batch: $batch_val, Stream_a: $stream_a_val =====" >> test1_2.log
        ...
        taskset -c 0,2,4,6,8 python -m sglang.bench_one_batch \
            ...
            --disable-radix-cache >> test1_2.log 2>&1 || true
    done
done
